...
//		2. It Defines an abstract class for creating families of related objects but without specifying their concrete sub-class.
//		3. This pattern is one of the creational pattern.
//		4. Abstract Factory pattern is almost similar to Factory Pattern is considered as another layer of abstraction over factory pattern.
//		5. Abstract Factory patterns work around a super-factory which creates other factories.
//		6. Abstract factory pattern implementation provides us with a framework that allows us to create objects that follow a general pattern.
//		7. So at runtime, the abstract factory is coupled with any desired concrete factory which can create objects of the desired type.
//
...
//
//		Why/When to use Abstract Factory design
...
public:
	Tire(string n, int presure) :
			name(n), presure(presure) {
	}
	;
	string getName() {
		return name;
...
public:
	Body(string n, int strength) :
			name(n), strength(strength) {
	}
	string getName() {
		return name;
	}
